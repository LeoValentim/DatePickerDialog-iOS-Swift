...
open class DatePickerDialog: UIView {
    ...
    public typealias CountDownCallback = ( TimeInterval? ) -> Void
    ...
    private var callbackCountDown: CountDownCallback?
    ...
    /// Create the CountDown dialog view, and animate opening the dialog
    open func showCountDown(_ title: String,
                            doneButtonTitle: String = "Done",
                            cancelButtonTitle: String = "Cancel",
                            defaultCountDown: TimeInterval = 0,
        ...
        self.datePickerMode = UIDatePickerMode.countDownTimer
        self.callbackCountDown = callback
        self.datePicker.datePickerMode = self.datePickerMode ?? UIDatePickerMode.countDownTimer
        self.datePicker.countDownDuration = defaultCountDown
        if let locale = self.locale {
            self.datePicker.locale = locale
        }
        /* Add dialog to main window */
    ...
    @objc func buttonTapped(sender: UIButton!) {
        if sender.tag == kDoneButtonTag {
            ...
            self.callbackCountDown?(self.datePicker.countDownDuration)
        } else {
            self.callback?(nil)
        }
